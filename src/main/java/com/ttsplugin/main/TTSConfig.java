...
public interface TTSConfig extends Config {
	...
	@ConfigItem(keyName = "voice", name = "Voice", description = "Voice for tts <br> Note: This wont do anything if random voice is enabled", position = 53, section = voiceSettings)
	default Voice voice() {
		return Voice.HAZEL;
	}
	
	@ConfigItem(keyName = "dialogVoice", name = "Dialog voice", description = "Voice for dialogs if Random voice is disabled and Dialogs is enabled in general settings <br> This only applies to the NPC not you. The voice above applies to you", position = 54, section = voiceSettings)
	default Voice dialogVoice() {
		return Voice.GEORGE;
	}
	
	@ConfigItem(keyName = "useDialogVoiceWithRandom", name = "Use dialog voice with random voice on", description = "Speaks all dialogs with the dialog voice even if random voice is enabled", position = 55, section = voiceSettings)
